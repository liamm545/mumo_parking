...
void PriusHybridPlugin::OnPriusCommand(const prius_msgs::Control::ConstPtr& msg) {
  ...
  double handCmd = (msg->steer < 0.) ? (msg->steer * -this->dataPtr->handWheelLow) : (msg->steer * this->dataPtr->handWheelHigh);

  handCmd = ignition::math::clamp(handCmd, this->dataPtr->handWheelLow, this->dataPtr->handWheelHigh);
...
void PriusHybridPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  this->dataPtr->node.Subscribe("/cmd_mode", &PriusHybridPlugin::OnCmdMode, this);
  ...
  this->dataPtr->handWheelJoint = this->dataPtr->model->GetJoint(handWheelJointName);
  ...
  unsigned int id = 0;
  this->dataPtr->flWheelRadius = this->CollisionRadius(this->dataPtr->flWheelJoint->GetChild()->GetCollision(id));
  this->dataPtr->frWheelRadius = this->CollisionRadius(this->dataPtr->frWheelJoint->GetChild()->GetCollision(id));
  this->dataPtr->blWheelRadius = this->CollisionRadius(this->dataPtr->blWheelJoint->GetChild()->GetCollision(id));
  this->dataPtr->brWheelRadius = this->CollisionRadius(this->dataPtr->brWheelJoint->GetChild()->GetCollision(id));
  ...
  ignition::math::Vector3d frCenterPos = this->dataPtr->frWheelJoint->GetChild()->GetCollision(id)->WorldPose().Pos();
  ignition::math::Vector3d blCenterPos = this->dataPtr->blWheelJoint->GetChild()->GetCollision(id)->WorldPose().Pos();
  ignition::math::Vector3d brCenterPos = this->dataPtr->brWheelJoint->GetChild()->GetCollision(id)->WorldPose().Pos();
  ...
  this->dataPtr->handWheelPID.Init(100, 0, 10, 0, 0, handWheelForce, -handWheelForce);
  ...
  this->dataPtr->updateConnection = event::Events::ConnectWorldUpdateBegin(std::bind(&PriusHybridPlugin::Update, this));

  this->dataPtr->keyboardSub = this->dataPtr->gznode->Subscribe("~/keyboard/keypress", &PriusHybridPlugin::OnKeyPress, this, true);

  this->dataPtr->worldControlPub = this->dataPtr->gznode->Advertise<msgs::WorldControl>("~/world_control");

  this->dataPtr->node.Subscribe("/keypress", &PriusHybridPlugin::OnKeyPressIgn, this);
}
...
void PriusHybridPlugin::OnCmdGear(const ignition::msgs::Int32& _msg) {
  ...
  this->dataPtr->directionState = static_cast<PriusHybridPluginPrivate::DirectionType>(state);
}
...
void PriusHybridPlugin::KeyControlTypeA(const int _key) {
  ...
  switch (_key) {
    ...
    case 113: {
      ...
      this->dataPtr->brakePedalPercent = std::min(this->dataPtr->brakePedalPercent, 1.0);
      this->dataPtr->lastPedalCmdTime = this->dataPtr->world->SimTime();
      break;
    }
    // a - steer left
    ...
    case 97: {
      ...
      this->dataPtr->handWheelCmd = std::min(this->dataPtr->handWheelCmd, this->dataPtr->handWheelHigh);
      this->dataPtr->lastSteeringCmdTime = this->dataPtr->world->SimTime();
      break;
    }
    // d - steer right
    ...
    case 100: {
      this->dataPtr->handWheelCmd -= 0.25;
      this->dataPtr->handWheelCmd = std::max(this->dataPtr->handWheelCmd, this->dataPtr->handWheelLow);
      this->dataPtr->lastSteeringCmdTime = this->dataPtr->world->SimTime();
      break;
    }
    // s - center steering
...
void PriusHybridPlugin::KeyControlTypeB(const int _key) {
  ...
  switch (_key) {
    ...
    case 115: {
      this->dataPtr->brakePedalPercent = 0.0;
      if (this->dataPtr->directionState != PriusHybridPluginPrivate::REVERSE) this->dataPtr->gasPedalPercent = 0.0;
      this->dataPtr->gasPedalPercent += 0.1;
      this->dataPtr->gasPedalPercent = std::min(this->dataPtr->gasPedalPercent, 1.0);
      this->dataPtr->directionState = PriusHybridPluginPrivate::REVERSE;
      this->dataPtr->lastPedalCmdTime = this->dataPtr->world->SimTime();
...
void PriusHybridPlugin::Update() {
  ...
  // F = -C |V|^2 V_hat
  auto dragForce = -dPtr->chassisAeroForceGain * dPtr->chassisLinearVelocity.SquaredLength() * dPtr->chassisLinearVelocity.Normalized();
  dPtr->chassisLink->AddForce(dragForce);

  // PID (position) steering
  this->dataPtr->handWheelCmd = ignition::math::clamp(this->dataPtr->handWheelCmd, -this->dataPtr->maxSteer / this->dataPtr->steeringRatio,
                                                      this->dataPtr->maxSteer / this->dataPtr->steeringRatio);
  double steerError = this->dataPtr->handWheelAngle - this->dataPtr->handWheelCmd;
  ...
  double tanSteer = tan(this->dataPtr->handWheelCmd * this->dataPtr->steeringRatio);
  this->dataPtr->flWheelSteeringCmd = atan2(tanSteer, 1 - this->dataPtr->frontTrackWidth / 2 / this->dataPtr->wheelbaseLength * tanSteer);
  this->dataPtr->frWheelSteeringCmd = atan2(tanSteer, 1 + this->dataPtr->frontTrackWidth / 2 / this->dataPtr->wheelbaseLength * tanSteer);
  ...
  double throttlePower = std::abs(flGasTorque * dPtr->flWheelAngularVelocity) + std::abs(frGasTorque * dPtr->frWheelAngularVelocity) +
                         std::abs(blGasTorque * dPtr->blWheelAngularVelocity) + std::abs(brGasTorque * dPtr->brWheelAngularVelocity);

  // auto release handbrake as soon as the gas pedal is depressed
  if (this->dataPtr->gasPedalPercent > 0) this->dataPtr->handbrakePercent = 0.0;

  double brakePercent = this->dataPtr->brakePedalPercent + this->dataPtr->handbrakePercent;
  // use creep braking if not in Neutral
  if (!neutral) {
    brakePercent = std::max(brakePercent, -creepPercent - this->dataPtr->gasPedalPercent);
  }

  brakePercent = ignition::math::clamp(brakePercent, 0.0, 1.0);
  dPtr->flWheelJoint->SetParam("friction", 0, dPtr->flJointFriction + brakePercent * dPtr->frontBrakeTorque);
  dPtr->frWheelJoint->SetParam("friction", 0, dPtr->frJointFriction + brakePercent * dPtr->frontBrakeTorque);
  dPtr->blWheelJoint->SetParam("friction", 0, dPtr->blJointFriction + brakePercent * dPtr->backBrakeTorque);
  dPtr->brWheelJoint->SetParam("friction", 0, dPtr->brJointFriction + brakePercent * dPtr->backBrakeTorque);
  ...
  else if (linearVel < this->dataPtr->speedMediumHigh && this->dataPtr->gasPedalPercent <= this->dataPtr->kGasPedalLowMedium) {
  ...
  if (regen) {
    ...
    batteryChargePower +=
        ...
        (dPtr->frontBrakeTorque * std::abs(dPtr->flWheelAngularVelocity) + dPtr->frontBrakeTorque * std::abs(dPtr->frWheelAngularVelocity) +
         dPtr->backBrakeTorque * std::abs(dPtr->blWheelAngularVelocity) + dPtr->backBrakeTorque * std::abs(dPtr->brWheelAngularVelocity));
  }
  dPtr->batteryCharge +=
      dt / 3600 * (batteryChargePower / dPtr->batteryChargeWattHours - batteryDischargePower / dPtr->batteryDischargeWattHours);
  ...
  // max value: 999.9
  double mpg = std::min(999.9, dPtr->odom / std::max(dPtr->gasConsumption, 1e-6));
...
void PriusHybridPlugin::UpdateHandWheelRatio() {
  ...
  double handWheelRange = this->dataPtr->handWheelHigh - this->dataPtr->handWheelLow;
  double high = 0.8727;
...
double PriusHybridPlugin::CollisionRadius(physics::CollisionPtr _coll) {
  ...
  if (_coll->GetShape()->HasType(gazebo::physics::Base::CYLINDER_SHAPE)) {
    physics::CylinderShape* cyl = static_cast<physics::CylinderShape*>(_coll->GetShape().get());
    return cyl->GetRadius();
  } else if (_coll->GetShape()->HasType(physics::Base::SPHERE_SHAPE)) {
    physics::SphereShape* sph = static_cast<physics::SphereShape*>(_coll->GetShape().get());
