...
GridMapGenerator::GridMapGenerator(ros::NodeHandlePtr nh)
    : nh_(nh), gridMapTopic_("grid_map"), mapResolution_(0.0), mapPositionX_(0.0), mapPositionY_(0.0), mapLength_(20.0), mapWidth_(20.0) {}
...
void GridMapGenerator::initRos() {
  ...
  polygonObstacleService_ = nh_->advertiseService("addPolygonObstacle", &GridMapGenerator::addPolygonObstacleService, this);
  circularObstacleService_ = nh_->advertiseService("addCircularObstacle", &GridMapGenerator::addCircularObstacleService, this);
...
void GridMapGenerator::initMap() {
  ...
  map_.setGeometry(grid_map::Length(mapLength_, mapWidth_), mapResolution_, grid_map::Position(mapPositionX_, mapPositionY_));
...
void GridMapGenerator::setCircleInMap(const std::string layerName, const grid_map::Position center, const double radius,
                                      const double value) {
...
grid_map::Polygon GridMapGenerator::convert(const se2_grid_map_generator_msgs::Polygon2D& polygon2d, const std::string& frameId) {
...
bool GridMapGenerator::addPolygonObstacleService(se2_grid_map_generator_msgs::AddPolygonObstacle::Request& req,
  ...
  if (length.x() < (2.0 * mapResolution_) || length.y() < (2.0 * mapResolution_)) {
    ROS_ERROR_STREAM("Bounding box of polygon with length " << length.x() << " or width " << length.y()
                                                            << " is too small for chosen map_resolution " << mapResolution_
...
bool GridMapGenerator::addCircularObstacleService(se2_grid_map_generator_msgs::AddCircularObstacle::Request& req,
  ...
  if ((2.0 * req.obstacle.circle.radius.data) < (2.0 * mapResolution_)) {
    ROS_ERROR_STREAM("Circle has a radius of " << req.obstacle.circle.radius.data << " which is too small for chosen map_resolution "
  ...
  // Set each given layer
  for (size_t idx = 0; idx < req.obstacle.layers.size(); ++idx) {
    setCircleInMap(req.obstacle.layers[idx].data, grid_map::Position(req.obstacle.circle.center.x.data, req.obstacle.circle.center.y.data),
                   req.obstacle.circle.radius.data, req.obstacle.values[idx].data);
  }
